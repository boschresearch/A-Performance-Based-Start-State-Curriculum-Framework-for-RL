...
from lib.start_selection.reaching_probability_map import generate_reach_prob_map_oracle, calc_spatial_reach_prob_grads, update_reach_prob_map
...
def run_fun(seed_list,
    ...
    prefix = 'sg_visited_grid_discrete'
    if grad_square:
        sq_arg = "sq"
    else:
        sq_arg = "nsq"
    ...
    for seed_val in seed_list:
        ...
        rpm_est = np.zeros(myTRPO.grid.rp_map.shape)
        rpm_est[25, 1] = 1.
        ...
        while eps_tot < num_outer_steps:
            ...
            rpm_est_save.append(copy.deepcopy(rpm_est))
            # print(rpm)

            rpm = (rpm_est > 0) * rpm
            rpm_red_save.append(copy.deepcopy(rpm))
            ...
            rpm_est = update_reach_prob_map(reach_prob_map=myTRPO.grid.rp_map,
        ...
        np.save(save_path + "rpm_est_" + prefix + "_" + str(seed_val) + "_" + str(grid_var) + "_" + suffix + "_" +
                str(config_dict['barcode']), rpm_est_np)

        rpm_red_np = np.array(rpm_red_save)
        np.save(save_path + "rpm_red_" + prefix + "_" + str(seed_val) + "_" + str(grid_var) + "_" + suffix + "_" +
                str(config_dict['barcode']), rpm_red_np)
