...
from lib.start_selection.reaching_probability_map import generate_reach_prob_map_oracle, calc_temporal_reach_prob_grads
...
def run_fun(seed_list,
    ...
    prefix = 'tpg_grid_discrete'
    if boltzmann_prob:
        boltzmann_arg = "bm"
    else:
        boltzmann_arg = "nbm"
    suffix = '%s_%i_%s_%s' % (lr_arg, grid_var, seed_arg, boltzmann_arg)
        ...
        while eps_tot < num_outer_steps:
            ...
            start_candidates, rp_grad_list_full, start_candidates_pos, rp_grad_list_pos, rp_grad_map = \
                calc_temporal_reach_prob_grads(
                    ...
                    reach_prob_map_new=rpm)
            rpm_old = copy.deepcopy(rpm)
