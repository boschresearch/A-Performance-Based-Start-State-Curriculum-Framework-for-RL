...
from lib.start_selection.reverse_curriculum import sample_nearby_continuous, OldStarts, \
    label_states_from_paths_continuous, select_starts
...
def run_fun(seed_list,
    ...
    prefix = 'rc_grid_continuous_usym'
    ...
    for seed_val in seed_list:
        ...
        starts = sample_nearby_continuous(grid=myTRPO.grid, starts=initial_start, N_new=N_new_val, T_B=T_B_val, M=M_val)
        old_starts = OldStarts(dist_threshold=0.1, init=tuple(myTRPO.grid.goal))
        ...
        while eps_tot < num_outer_steps:
            ...
            starts = sample_nearby_continuous(grid=myTRPO.grid, starts=starts, N_new=N_new_val, T_B=T_B_val, M=M_val)
            print("NEW STARTS")
            # print(starts)
            ...
            states, labels = label_states_from_paths_continuous(path_trajectories=s_list, path_successes=r_list)
            print("STATES")
            # print(states)
            print("LABELS")
            # print(labels)
            ...
            if not starts:
                if train_success < 0.5:
                    ...
                    print("GOOD")
                    starts = sample_nearby_continuous(grid=myTRPO.grid,
                                                      starts=rho,
                                                      N_new=N_new_val,
                                                      T_B=10 * T_B_val,
                                                      M=M_val,
