...
from lib.start_selection.SaggRIAC import SaggRIAC
from lib.start_selection.reaching_probability_map import compute_competences
...
def run_fun(seed_list,
    ...
    prefix = 'sagg_riac_grid_continuous'
    suffix = '%s_%i_%s' % (lr_arg, grid_var, start_arg)
    ...
    for seed_val in seed_list:
        ...
        mySaggRIAC = SaggRIAC(state_size=2,
                              state_bounds=np.array([[-0.5, -0.5], [29.5, 19.5]]),
                              max_goals=100,
                              max_history=100
                              )
        ...
        while eps_tot < num_outer_steps:
            ...
            new_starts = mySaggRIAC.sample_states(num_samples=50)
            print(new_starts)

            _, result, s_list, a_list, r_list, st_list, _ = myTRPO.train(start_p_list=new_starts,
            ...
            starts, competences = compute_competences(s_list, r_list, (25., 1.))
            print(starts)
            print(competences)

            mySaggRIAC.add_states(starts, competences)
