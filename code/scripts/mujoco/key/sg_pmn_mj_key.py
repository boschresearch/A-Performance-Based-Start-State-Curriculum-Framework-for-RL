...
from lib.trpo.trpo_mujoco_key_sg_direct import TRPOTrainer
from lib.start_selection.select_start_continuous import sg_direct_mj_key
from lib.start_selection.select_start import start_state_sampling_probabilities_from_grads_cont
...
def run_fun(seed_list,
    ...
    for seed_val in seed_list:
        ...
        while eps_tot < num_outer_steps:
            ...
            if i > 1:
                start_candidates, rp_grad_list = sg_direct_mj_key(trpo=myTRPO)
                train_starts, sampling_prob_list = start_state_sampling_probabilities_from_grads_cont(
                    start_candidates=start_candidates,
                    rp_grads=rp_grad_list)
                print(train_starts)
            else:
                train_starts = [(1.55, 0.4, -3.75, -1.15, 1.81, -2.09, 0.05)]
