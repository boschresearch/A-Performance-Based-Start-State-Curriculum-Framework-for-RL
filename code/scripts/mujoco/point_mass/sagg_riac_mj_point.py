...
from lib.start_selection.reaching_probability_map import compute_competences_mj
...
def run_fun(seed_list,
    ...
    for seed_val in seed_list:
        ...
        while eps_tot < num_outer_steps:
            ...
            new_starts = mySaggRIAC.sample_states(num_samples=300)
            print(new_starts)

            _, result, s_list, a_list, r_list, st_list = myTRPO.train(start_p_list=new_starts,
                                                                      goal_p=myTRPO.grid.goal,
                                                                      sampling_probs=[],
                                                                      sampling_mode=False)

            starts, competences = compute_competences_mj(s_list, r_list, myTRPO.grid.goal)
            print(starts)
            print(competences)
