...
def get_flat_params_from(model):
    ...
    for param in model.parameters():
        params.append(param.data.contiguous().view(-1))
    ...
    for param in inputs:
        if grad_grad:
            ...
            if param.grad is None:
                grads.append(Variable(torch.zeros(param.data.contiguous().view(-1).shape)))
            else:
...
def compute_flat_grad(output, inputs, filter_input_ids=set(),
    ...
    for i, param in enumerate(inputs):
        if i in filter_input_ids:
            out_grads.append(Variable(torch.zeros(param.data.contiguous().view(-1).shape)))
        else:
