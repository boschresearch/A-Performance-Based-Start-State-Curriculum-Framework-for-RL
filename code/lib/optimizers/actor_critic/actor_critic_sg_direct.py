...
class ActorCriticOptimizer(Optimizer):
    def __init__(self, policy, discount=0.998, gae_lambda=0.95, l2_reg=0, imp_weight=False, **kwargs):
    ...
    def _update_networks(self, policy,
        ...
        values_nd = self.networks["critic_nd"](Variable(states[:, :7], volatile=True)).data
        # values_nd = self.networks["critic_nd"](Variable(states, volatile=True)).data
        print(values.shape)
        ...
        _, returns_nd = gae(
            rewards, masks, values, discount=1.0,
            gae_lambda=self.gae_lambda, use_gpu=self.use_gpu)

        optimizer_str_list = [key for key in
                              ["policy", "critic", "critic_nd"][:len(self.optimizers)]]
        optimizers = [self.optimizers[key] for key in optimizer_str_list]
        self.step(policy, self.networks["critic"], self.networks["critic_nd"], *optimizers[:3],
                  states, actions, returns, returns_nd, advantages)
        return policy

    @classmethod
    def _init_networks(cls, obs_dim, action_dim):
        return {"critic": StateValue(obs_dim), "critic_nd": StateValue(7)}
        # return {"critic": StateValue(obs_dim), "critic_nd": StateValue(obs_dim)}
...
class TRPO(ActorCriticOptimizer):
    def __init__(self, policy, max_kl=1e-1, damping=1e-2, use_fim=True,
        ...
        self.transition_loss_list = []

    def step(self, policy_net, value_net, value_net_nd, states, actions, returns, returns_nd, advantages, weights=None):

        """update critic"""
        values_target = Variable(returns)
        values_target_nd = Variable(returns_nd)

        values_pred_ndx = value_net_nd(Variable(states[:, :7]))
        # values_pred_ndx = value_net_nd(Variable(states))
        value_loss_ndx = (values_pred_ndx - values_target_nd).pow(2).mean().data.numpy().squeeze()
        self.current_value_loss = copy.deepcopy(value_loss_ndx)
        ...
        def get_value_loss_nd(flat_params_nd):
            torch_utils.set_flat_params_to(value_net_nd,
                                           torch_utils.torch.from_numpy(flat_params_nd))
            for param in value_net_nd.parameters():
                if param.grad is not None:
                    param.grad.data.fill_(0)
            values_pred_nd = value_net_nd(Variable(states[:, :7]))
            # values_pred_nd = value_net_nd(Variable(states))
            value_loss_nd = (values_pred_nd - values_target_nd).pow(2).mean()

            # weight decay
            for param in value_net_nd.parameters():
                value_loss_nd += param.pow(2).sum() * self.l2_reg
            value_loss_nd.backward()

            return value_loss_nd.data.cpu().numpy()[0], \
                   torch_utils.get_flat_grad_from(
                       value_net_nd.parameters()).data.cpu().numpy(). \
                       astype(np.float64)

        flat_params_nd, _, opt_info = scipy.optimize.fmin_l_bfgs_b(
            get_value_loss_nd,
            torch_utils.get_flat_params_from(value_net_nd).cpu().numpy(), maxiter=25)
        torch_utils.set_flat_params_to(value_net_nd, torch.from_numpy(flat_params_nd))
        ...
        def get_loss(volatile=False):
            ...
            if True:
                ...
                entropy_loss = - policy_net.get_entropy(Variable(states, volatile=volatile), Variable(actions))
                return action_loss.mean() - 0.01 * entropy_loss.mean()
