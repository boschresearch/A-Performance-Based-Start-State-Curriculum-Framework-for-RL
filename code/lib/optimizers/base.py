...
class Optimizer(object):
    ...
    def process_batch(self, policy, batch, update_policy_args):
        # states, actions, rewards, masks, weights = self._unpack_batch(batch)
        states, actions, rewards, masks = self._unpack_batch(batch)
        # policy.bn.update_state_stats(states)
        if self.use_gpu:
        ...
        policy = self._update_networks(
            ...
            batch["num_episodes"], weights, *update_policy_args)
        """
        policy = self._update_networks(
            policy, actions, masks, rewards, states,
    ...
    def _unpack_batch(batch):
        ...
        # weights_np = np.array(batch["weight"], dtype=np.float32)
        # weights_np_norm = (weights_np / np.sum(weights_np)) * np.sum(np.ones_like(weights_np))
        # weights = torch.from_numpy(weights_np_norm)
        # weights = None
